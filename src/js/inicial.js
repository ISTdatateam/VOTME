...
const DEFAULT_INITIAL_EVAL_PATHS = [
    "FAENA-EVISCERADO-AURICULOCELULITIS-Metodo Art tool.xlsx",
    ...
    "FAENA-EVISCERADO-RECOLGADO-Metodo Art tool.xlsx",
    "FAENA-EVISCERADO-REPASE DE CONTRE-Metodo Art tool.xlsx",
    "FAENA-EVISCERADO-REPASE DE PANAS-Metodo Art tool.xlsx",
...
function groupColumnsByTop(headers, topHeaders, startIdx, endIdx) {
  ...
  for (let i = start; i < end; i++) {
    ...
    if (normLabel.includes("condicion aceptable") || normLabel.includes("condición aceptable") || normLabel.includes("condicion critica") || normLabel.includes("condición crítica") || normLabel.includes("resultado")) {
      continue;
    }
...
/* -------------- PARSEO DE HojaResultado CON MATRIZ O8:W26 -------------- */
function parseInitialEvalSheet(rows, sourceName) {
  ...
  // MATRIZ MOVIMIENTO REPETITIVO: HojaResultado!O8:W26 (0-based: filas 7..25, col 14..22)
  let movMatrix = [];
  for (let r = 7; r <= 25 && r < rows.length; r++) {
    const row = rows[r] || [];
    const slice = [];
    for (let c = 14; c <= 22; c++) {
      slice.push(row[c] ?? "");
    }
    if (slice.some((cell) => String(cell ?? "").trim() !== "")) {
      movMatrix.push(slice);
    }
  }
...
async function resolveInitialEvalPaths() {
  ...
  const configured = Array.isArray(window.INITIAL_EVAL_FILES) && window.INITIAL_EVAL_FILES.length ? window.INITIAL_EVAL_FILES : [];
...
function wireUI() {
  ...
  el("perPage").addEventListener("change", () => {
    STATE.perPage = parseInt(el("perPage").value, 10) || 10;
    STATE.page = 1;
    render();
  });
  ...
  if (movSheetName) {
    ...
    if (wsMov) {
      ...
      if (rows2d.length) {
        ...
        const headerRow = rows2d[1] && rows2d[1].some((x) => String(x || "").trim() !== "") ? rows2d[1] : rows2d[0];
...
function buildAdvancedActionLabel(entries) {
  ...
  if (anyNoAcceptable) {
    return {
      ...
      title: anyCritical ? "Identificación avanzada trabajo repetitivo: Crítico" : "Identificación avanzada trabajo repetitivo: No crítico",
      message: anyCritical ? "Intervenir tarea 90 días y re-identificar." : "Se requiere realizar identificación inicial.",
    };
  }
  return {
    severity: "ok",
...
function populateFactor() {
  ...
  sel.innerHTML =
    `<option value="">(Todos)</option>` + RISKS.map((r) => `<option value="${r.key}">${escapeHtml(r.label)}</option>`).join("");
}
...
function classifyMovRep(p, w) {
  ...
  if (t.includes("acept") || t.includes("bajo") || t.includes("sin riesgo")) return { cls: "status-ok", label: "Aceptable" };
...
function factorChips(r) {
  ...
  for (const rf of RISKS) {
    ...
    parts.push(
      `<span class="${cls}" title="${escapeHtml(rf.label)}">${st}${ico} <span>${escapeHtml(rf.label)}</span></span>`
    );
  }
  if (!parts.length) {
...
function renderAdvancedActionLabel(info) {
  if (!info) return "";
  const ico =
    info.severity === "bad" ? "bi-exclamation-octagon-fill" : info.severity === "warn" ? "bi-exclamation-triangle-fill" : "bi-patch-check-fill";
  return `
...
/* ======= CATEGORÍA DE EXPOSICIÓN (BAJO/MEDIO/ALTO) ======= */
function getExpoCategoryInfo(puntajeExpo) {
  if (puntajeExpo == null || puntajeExpo === "") return null;
  const num = Number(String(puntajeExpo).replace(",", "."));
  if (Number.isNaN(num)) return null;
  if (num <= 11) return { label: "Bajo", className: "expo-low", color: "#00b050" };
  if (num <= 21) return { label: "Medio", className: "expo-med", color: "#ffc000" };
  return { label: "Alto", className: "expo-high", color: "#ff0000" };
}

/* ======= Evaluación inicial en tarjeta (snippet) ======= */
function renderInitialEvalSnippet(ev) {
  if (!ev) return "";
  const parts = [];

  // 1) Puntaje de la tarea
  if (ev.puntajeTarea != null) {
    parts.push(`
      <div class="iev-item">
        <div class="iev-label">Puntaje tarea</div>
        <div class="iev-value">${escapeHtml(ev.puntajeTarea)}</div>
      </div>
    `);
  }

  // 2) Puntaje de exposición total (solo número)
  if (ev.puntajeExpo != null) {
    parts.push(`
      <div class="iev-item">
        <div class="iev-label">Puntaje exposición total</div>
        <div class="iev-value">${escapeHtml(ev.puntajeExpo)}</div>
      </div>
    `);
  }

  // 3) Categoría de duración (Bajo/Medio/Alto) con color
  const cat = getExpoCategoryInfo(ev.puntajeExpo);
  if (cat) {
    const pill = `
      <span class="expo-pill ${cat.className}"
            style="display:inline-block;padding:0.1rem 0.6rem;border-radius:999px;
                   background-color:${cat.color};color:#fff;font-weight:600;">
...
/* ======================================================================
   TABLA O8:W26 – MOVIMIENTO REPETITIVO (DETALLE)
   ====================================================================== */

/** Separa texto "Color / Puntaje" o variantes en { color, score } */
function splitColorScore(raw) {
  const s = String(raw ?? "").trim();
  if (!s) return { color: "", score: "" };

  // Solo número
  if (/^-?\d+(?:[.,]\d+)?$/.test(s)) {
    return { color: "", score: s };
  }
  ...
  if (parts.length >= 2) {
    ...
    const numMatch = parts
      .slice(1)
      .join("/")
      .match(/-?\d+(?:[.,]\d+)?/);
    const score = numMatch ? numMatch[0].replace(",", ".") : "";
    return { color, score };
  }

  // Color + número sin slash
...
/** Devuelve estilos de fondo según nombre del color */
function movColorStyle(name) {
  const n = toLowerNoAccents(name || "");
  if (!n) return "";
  ...
  if (n.startsWith("amb")) {
    // ámbar / ambar
    return "background-color:#ffe699;color:#7f6000;font-weight:600;";
  }
  if (n.startsWith("roj")) {
    // rojo
...
/* ======= Tabla O8:W26 para Movimiento repetitivo en detalle ======= */
function renderMovInitialMatrix(ev) {
  if (!ev || !Array.isArray(ev.movMatrix) || !ev.movMatrix.length) return "";

  let matrix = ev.movMatrix;

  // Si la primera fila trae cabecera ("Factor de riesgo"), se descarta del cuerpo
  if (matrix.length && toLowerNoAccents(String(matrix[0][0] || "")).includes("factor")) {
  ...
  for (const rawRow of matrix) {
    ...
    // O:S están unidas en Excel → el texto del factor puede quedar en cualquiera de las primeras 5 columnas del rango O:W
    const factor = row
      .slice(0, 5)
      .find((v) => String(v || "").trim() !== "") || "";

    // Columnas del rango pensadas como:
    // [0..4]  => Factor de riesgo (celdas fusionadas O:S)
    // [5]     => Brazo Izq - Color (texto o "Color / N")
    // [6]     => Brazo Izq - Puntaje
    // [7]     => Brazo Der - Color (texto o "Color / N")
    // [8]     => Brazo Der - Puntaje
    const leftRaw = row[5] ?? "";
    const leftScoreRaw = row[6] ?? "";
    const rightRaw = row[7] ?? "";
    const rightScoreRaw = row[8] ?? "";

    // Fila totalmente vacía → se ignora
    if (
      !String(factor).trim() &&
      !String(leftRaw).trim() &&
      !String(leftScoreRaw).trim() &&
      !String(rightRaw).trim() &&
      !String(rightScoreRaw).trim()
    ) {
      continue;
    }

    const leftSplit = splitColorScore(leftRaw);
    const rightSplit = splitColorScore(rightRaw);
    const leftScoreSplit = splitColorScore(leftScoreRaw);
    const rightScoreSplit = splitColorScore(rightScoreRaw);

    const leftScore = leftScoreSplit.score || leftSplit.score || leftScoreRaw || "";
    const rightScore = rightScoreSplit.score || rightSplit.score || rightScoreRaw || "";

    rows.push({
      factor,
      leftColor: leftSplit.color || leftRaw,
      leftScore,
      rightColor: rightSplit.color || rightRaw,
      rightScore,
    });
  }
  ...
  const headerTop = `
    <tr>
      <th rowspan="2">Factor de riesgo</th>
      <th colspan="2">Brazo izquierdo</th>
      <th colspan="2">Brazo derecho</th>
    </tr>
    <tr>
      <th>Color</th>
      <th>Puntaje</th>
  ...
  const bodyHtml = rows
    .map((r) => {
      const leftColorText = String(r.leftColor || "").trim();
      const rightColorText = String(r.rightColor || "").trim();
      const leftScoreText = String(r.leftScore || "").trim();
      const rightScoreText = String(r.rightScore || "").trim();

      const leftStyle = movColorStyle(leftColorText);
      const rightStyle = movColorStyle(rightColorText);

      const leftColorCell = leftColorText ? `<td style="${leftStyle}">${escapeHtml(leftColorText)}</td>` : "<td></td>";
      const rightColorCell = rightColorText ? `<td style="${rightStyle}">${escapeHtml(rightColorText)}</td>` : "<td></td>";

      const leftScoreCell = leftScoreText
        ? `<td class="text-center">${escapeHtml(leftScoreText)}</td>`
        : '<td class="text-center"></td>';
      const rightScoreCell = rightScoreText
        ? `<td class="text-center">${escapeHtml(rightScoreText)}</td>`
        : '<td class="text-center"></td>';
  ...
  const meta = infoLine.length ? `<div class="iev-meta mt-2">${infoLine.join(" · ")}</div>` : "";

  const noteSrc = ev.comentarios || ev.descripcion || "";
  const note = noteSrc ? `<div class="iev-note mt-2"><i class="bi bi-info-circle"></i> ${escapeHtml(noteSrc)}</div>` : "";

  return `
    <div class="detail-card">
      <h6 class="section-title mb-2">
        <i class="bi bi-clipboard-check"></i> Evaluación inicial · Movimiento repetitivo
      </h6>
      <div class="table-responsive">
        <table class="table table-sm table-bordered mov-eval-table">
...
/* ======= Evaluación inicial en detalle (modal) ======= */
function renderInitialEvalDetail(ev, factorLabel) {
  if (!ev) return "";

  const factorNorm = toLowerNoAccents(factorLabel || "");

  // Caso especial: Movimiento repetitivo → usar matriz O8:W26
  if (factorNorm.includes("movimiento repetitivo") && Array.isArray(ev.movMatrix) && ev.movMatrix.length) {
    return renderMovInitialMatrix(ev);
  }
  ...
  const title = factorLabel ? `Evaluación inicial · ${escapeHtml(factorLabel)}` : "Evaluación inicial (HojaResultado)";
    ...
    acceptable: [
      {
        ...
        entries: [
          ...
          mk(
            "En caso de haber flexión de tronco (inclinación hacia delante), ¿es menor a 20º?, o en caso de existir extensión (inclinación hacia atrás), ¿el tronco está totalmente apoyado?"
          ),
...
function renderPosturaTab(post, summaryCards) {
  ...
  const acceptable =
    structure.acceptable.map((sec) => renderPosturaSection(sec, row)).filter(Boolean).join("") ||
...
function renderMmcTab(data, structure, headers, sheetLabel, summaryCards) {
  const summaryBlock = renderTabSummary(summaryCards);
  if (!structure) {
    return `<div class="postura-tab">${summaryBlock}<div class="alert alert-warning"><i class="bi bi-exclamation-triangle"></i>No se pudo interpretar la hoja “${escapeHtml(
      sheetLabel
    )}”.</div></div>`;
  }
  if (!data) {
    return `<div class="postura-tab">${summaryBlock}<div class="alert alert-warning"><i class="bi bi-exclamation-triangle"></i>No se encontraron coincidencias en la hoja “${escapeHtml(
      sheetLabel
    )}”.</div></div>`;
  }
  ...
  const acceptableBlock =
    renderGroupedSections(structure.acceptableGroups, row) ||
    `<div class="alert alert-light border text-muted"><i class="bi bi-info-circle"></i> Sin respuestas para condición aceptable.</div>`;
  ...
  const criticalBlock =
    hasCriticalGroups || skipCritical
      ? `<div class="group-block mt-4">
        <div class="group-title text-uppercase small text-muted fw-bold mb-2">Condición Crítica</div>
        ${criticalContent}
      </div>`
...
function cardHtml(r, idx) {
  ...
  const summaryCards = [];
  const addSummary = (html) => {
    if (html) summaryCards.push(html);
  };
  if (factorPresent.mov) {
    ...
    addSummary(
      renderStateCard("Trabajo repetitivo de miembros superiores · Condición crítica", movCritical, "bi-exclamation-diamond-fill")
    );
  }
...
const SKIP_LABELS = new Set(["mujeres", "col2", "col3", "col4", "col5", "col6", "col7", "col8", "col9", "n°", "n."]);
...
function renderMovRepTab(mov, summaryCards, actionHtml) {
  ...
  const acceptableBlock =
    renderGroupedSections(MOVREP_STRUCTURE.acceptableGroups, row) ||
    `<div class="alert alert-light border text-muted"><i class="bi bi-info-circle"></i> Sin respuestas para condición aceptable.</div>`;
...
function openDetail(r) {
  ...
  if (factorPresent.mmcLev && MMC_LEV_STRUCTURE) {
    ...
    addSummary(
      "mmc-lev",
      renderStateCard("MMC Levantamiento/Descenso · Condición crítica", mmcLev?.condCritica, "bi-exclamation-octagon-fill")
    );
  }
  if (factorPresent.mmcEmp && MMC_EMP_STRUCTURE) {
    ...
    addSummary(
      "mmc-emp",
      renderStateCard("MMC Empuje/Arrastre · Condición crítica", mmcEmp?.condCritica, "bi-exclamation-triangle-fill")
    );
  }
...
`;
  ...
  if (!tabs.length) {
    if (disabledTabs.length) {
      const disabledNav = disabledTabs
        .map(
          (title) => `
    ...
    const navActive = tabs
      .map(
        (tab, idx) => `
      <li class="nav-item" role="presentation">
        <button class="nav-link${idx === 0 ? " active" : ""}" id="detail-tab-${tab.id}" data-bs-toggle="tab" data-bs-target="#detail-pane-${
          tab.id
        }" type="button" role="tab" aria-controls="detail-pane-${tab.id}" aria-selected="${idx === 0 ? "true" : "false"}">
